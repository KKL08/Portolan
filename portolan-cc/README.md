...
| `/portolan:orchestrate` | 编排循环（自动进入；中断后手动接续） |
...
**`/portolan:orchestrate`** — 编排循环。任务派发后由主 session 自动运转：派执行 subagent、盯终态、按档位决定重派还是找人，每一步分支都由 `state-guard orch-step` 纯代码判定，不靠对话记忆。发起流程走完会自动进入，平时不用手动调；主 session 意外中断后，用它在新会话里把编排循环接上。
