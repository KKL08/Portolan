...
**冻结哈希重算规则**：任务协议单/rubric.md 的每次合法写入（初次确认落盘、决议记录追加、协议修订——只有 portolan 三模式有权写入）完成后，立即调用 `state-guard update-freeze` 重算 SHA-256 冻结哈希并更新工作底稿"冻结哈希"节。执行环无权写入这些文件，因此执行期间的哈希漂移必然是篡改，finish 第零步据此判定。
